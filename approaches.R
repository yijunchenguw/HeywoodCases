...
cfa_log_likelihood_test <- function(lambda, theta_delta, phi, sample_cov, n) {
  #theta_delta_diag <- torch_diag(theta_delta)  # Ensure theta_delta is diagonal
  theta_delta_diag <- torch_diag(theta_delta) 
  lambda <- lambda*lambda_mask
  #phi_diag <- torch_eye(n_factors)
...
cfa_log_likelihood_map_exp <- function(lambda, theta_delta, phi, sample_cov, n) {
  #theta_delta_diag <- torch_diag(theta_delta)  # Ensure theta_delta is diagonal
...
cfa_log_likelihood_sigmoid_exp <- function(lambda, theta_delta, phi, sample_cov, n) {
  #theta_delta_diag <- torch_diag(theta_delta)  # Ensure theta_delta is diagonal
  theta_delta_diag <- torch_diag(exp(theta_delta)) 
  phi_diag <- phi.matrix(phi)$requires_grad_(T)
...
cfa_log_likelihood_exp <- function(lambda, theta_delta, phi, sample_cov, n) {
  #theta_delta_diag <- torch_diag(theta_delta)  # Ensure theta_delta is diagonal
  theta_delta_diag <- torch_diag(exp(theta_delta)) 
  lambda <- lambda*lambda_mask
...
mle_method <- function(datb, method = 3, tolerance = 1e-4, n){
...
# Initializing theta_delta with the diagonal of sample covariance
  if(method == 3) {
  initial_lambda_values[lambda_structure == 1] <- 0.1 
  cfa_function <- cfa_log_likelihood_exp
  }else if(method == 4) {
    initial_lambda_values[lambda_structure == 1] <- inverse_sigmoid(0.1)
...
phi <- torch_tensor(0.3, requires_grad = TRUE)
...
for (epoch in 1:num_epochs) {
  ...
  loss <- cfa_function(lambda, theta_delta, phi, sample_cov_tensor, n)  # Compute loss
  ...
  if (!is.null(last_loss) && abs(last_loss - loss$item()) < tolerance) {
    ...
    result <- list(lambda = as.matrix(lambda), se = as.matrix(se),  upper = as.matrix(ci_95_upper), lower = as.matrix(ci_95_lower),theta = as.matrix(exp(theta_delta)), phi = as.matrix(phi.matrix(phi)), ll = as.numeric(loss$item()))
    result
    break
  }
